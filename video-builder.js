...
const MAX_CLIPS=5, MAX_TOTAL_SECONDS=120, MAX_CLIP_SECONDS=120, MIN_CLIP_SECONDS=.25;
...
function addUI(){
  ...
  section.innerHTML=`
  <div class="sectionTitle"><div><h2>Video Clip Builder</h2><p>Join up to five video clips, up to two minutes total, and add background music.</p></div></div>
  <div class="grid2"><div class="card">
    ...
    <div class="field"><label for="videoClipUpload">Upload 1–5 video clips</label><input id="videoClipUpload" type="file" accept="video/*" multiple><small>Each clip may be up to 2 minutes. The finished video may be up to 2 minutes total. Clips play in the order shown below.</small></div>
  ...
  const grid=$('.quickGrid');if(grid&&!grid.querySelector('[data-jump="videoBuilder"]')){const q=document.createElement('button');q.className='card quick';q.dataset.jump='videoBuilder';q.innerHTML='<strong>Video Clip Builder</strong><span>Join clips into a video up to two minutes long.</span>';q.onclick=()=>showView('videoBuilder');grid.insertBefore(q,grid.children[2]||null)}
}
...
function totalDuration(){return state.clips.reduce((n,c)=>n+c.duration,0)}
function renderClips(){const box=$('#clipList');if(!box)return;box.innerHTML=state.clips.length?'':'<div class="empty">No video clips selected.</div>';state.clips.forEach((c,i)=>{const row=document.createElement('div');row.className='downloadItem';row.innerHTML=`<div><h3>${i+1}. ${c.file.name}</h3><small>${c.duration.toFixed(1)} seconds</small></div>`;const acts=document.createElement('div');acts.className='actions';const up=document.createElement('button');up.textContent='↑';up.disabled=i===0;up.onclick=()=>{[state.clips[i-1],state.clips[i]]=[state.clips[i],state.clips[i-1]];renderClips()};const down=document.createElement('button');down.textContent='↓';down.disabled=i===state.clips.length-1;down.onclick=()=>{[state.clips[i+1],state.clips[i]]=[state.clips[i],state.clips[i+1]];renderClips()};const remove=document.createElement('button');remove.textContent='Remove';remove.className='danger';remove.onclick=()=>{state.clips.splice(i,1);renderClips();drawEmpty();$('#clipBuildStatus').textContent=state.clips.length?`${state.clips.length} clip${state.clips.length===1?'':'s'} ready · ${totalDuration().toFixed(1)} of 120 seconds.`:'No clips selected.'};acts.append(up,down,remove);row.appendChild(acts);box.appendChild(row)});if(state.clips.length){const total=document.createElement('div');total.className='notice';total.textContent=`Total length: ${totalDuration().toFixed(1)} of 120 seconds`;box.appendChild(total)}}
...
async function exportVideo(){
  ...
  const total=totalDuration();if(total>MAX_TOTAL_SECONDS+.05){$('#clipBuildStatus').textContent='The finished video must be two minutes or shorter.';return}
...
function init(){addUI();bind();resize();renderClips();document.querySelectorAll('.badge').forEach(el=>{if(el.textContent.includes('Version'))el.textContent='Version 0.8.2'});const footer=$('.footer');if(footer)footer.textContent='Five Oaks Studio — Version 0.8.2'}
