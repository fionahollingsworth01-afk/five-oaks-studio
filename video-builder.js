...
function addUI(){
  ...
  section.innerHTML=`
  <div class="sectionTitle"><div><h2>Video Clip Builder</h2><p>Blend up to five video clips into one smooth video up to three minutes long.</p></div></div>
  <div class="grid2"><div class="card">
    ...
    <div class="field"><label for="videoClipUpload">Upload 1–5 video clips</label><input id="videoClipUpload" type="file" accept="video/*" multiple><small>Each clip may be up to 3 minutes. The finished video may be up to 3 minutes total. Clips play in the order shown below.</small></div>
    <div id="clipList" class="downloadList"></div>
    <div class="field"><label><input id="blendClips" type="checkbox" checked style="width:auto"> Smoothly blend clips together</label><small>A gentle crossfade hides the hard cut between clips.</small></div>
  ...
  const grid=$('.quickGrid');if(grid&&!grid.querySelector('[data-jump="videoBuilder"]')){const q=document.createElement('button');q.className='card quick';q.dataset.jump='videoBuilder';q.innerHTML='<strong>Video Clip Builder</strong><span>Blend clips into a video up to three minutes long.</span>';q.onclick=()=>showView('videoBuilder');grid.insertBefore(q,grid.children[2]||null)}
}
...
function totalDuration(){let total=rawDuration();for(let i=0;i<state.clips.length-1;i++)total-=transitionFor(state.clips[i],state.clips[i+1]);return Math.max(0,total)}
function updateReadyStatus(){const total=totalDuration();$('#clipBuildStatus').textContent=state.clips.length?`${state.clips.length} clip${state.clips.length===1?'':'s'} ready · ${total.toFixed(1)} of 180 seconds${blendEnabled()&&state.clips.length>1?' · smooth blending on':''}.`:'No clips selected.'}
function renderClips(){const box=$('#clipList');if(!box)return;box.innerHTML=state.clips.length?'':'<div class="empty">No video clips selected.</div>';state.clips.forEach((c,i)=>{const row=document.createElement('div');row.className='downloadItem';row.innerHTML=`<div><h3>${i+1}. ${c.file.name}</h3><small>${c.duration.toFixed(1)} seconds</small></div>`;const acts=document.createElement('div');acts.className='actions';const up=document.createElement('button');up.textContent='↑';up.disabled=i===0;up.onclick=()=>{[state.clips[i-1],state.clips[i]]=[state.clips[i],state.clips[i-1]];renderClips();updateReadyStatus()};const down=document.createElement('button');down.textContent='↓';down.disabled=i===state.clips.length-1;down.onclick=()=>{[state.clips[i+1],state.clips[i]]=[state.clips[i],state.clips[i+1]];renderClips();updateReadyStatus()};const remove=document.createElement('button');remove.textContent='Remove';remove.className='danger';remove.onclick=()=>{state.clips.splice(i,1);renderClips();drawEmpty();updateReadyStatus()};acts.append(up,down,remove);row.appendChild(acts);box.appendChild(row)});if(state.clips.length){const total=document.createElement('div');total.className='notice';total.textContent=`Finished length: ${totalDuration().toFixed(1)} of 180 seconds${blendEnabled()&&state.clips.length>1?' with smooth crossfades':''}`;box.appendChild(total)}}
function resize(){const c=$('#clipBuildCanvas');if(!c)return;[c.width,c.height]=state.format==='wide'?[1280,720]:[720,1280];drawEmpty()}
function drawEmpty(){const c=$('#clipBuildCanvas');if(!c)return;const x=c.getContext('2d');x.fillStyle='#080605';x.fillRect(0,0,c.width,c.height);x.fillStyle='#bfae9f';x.textAlign='center';x.font='30px system-ui';x.fillText(state.clips.length?`${state.clips.length} clip${state.clips.length===1?'':'s'} · ${totalDuration().toFixed(1)} sec`:'Upload video clips',c.width/2,c.height/2)}
...
async function playSequence(recording=false){
  ...
  const canvas=$('#clipBuildCanvas'),ctx=canvas.getContext('2d'),items=[];
  state.busy=true;
  try{
    ...
    for(let i=0;i<items.length;i++){
      if(!state.busy)break;
      current=items[i];const next=items[i+1]||null;const transition=next?transitionFor(current.clip,next.clip):0;const blendStart=Math.max(0,current.clip.duration-transition);
      await runFrames(()=>{drawFrame(ctx,canvas,current.v);return current.v.ended||current.v.currentTime>=blendStart-.02});
      if(!state.busy)break;
      if(next&&transition>0){
        next.v.currentTime=0;await next.v.play();
        await runFrames(()=>{const alpha=next.v.currentTime/transition;drawFrame(ctx,canvas,current.v,next.v,alpha);return next.v.currentTime>=transition-.02||next.v.ended});
        current.v.pause();
      }else if(next){
        current.v.pause();next.v.currentTime=0;await next.v.play();
      }else{
        await runFrames(()=>{drawFrame(ctx,canvas,current.v);return current.v.ended||current.v.currentTime>=current.clip.duration-.02});
        current.v.pause();
      }
    }
  }finally{
    items.forEach(item=>{item.v.pause();URL.revokeObjectURL(item.url)});
    state.busy=false;
  }
}
...
async function exportVideo(){
  ...
  const total=totalDuration();if(total>MAX_TOTAL_SECONDS+.05){$('#clipBuildStatus').textContent='The finished video must be three minutes or shorter.';return}
  ...
  const b=$('#exportClipBuild');b.disabled=true;$('#clipBuildStatus').textContent='Creating your finished video… Keep this page open until it downloads.';
  const canvas=$('#clipBuildCanvas'),videoStream=canvas.captureStream(30),audioCtx=new (window.AudioContext||window.webkitAudioContext)(),dest=audioCtx.createMediaStreamDestination(),music=makeMusic(total,audioCtx,dest),stream=new MediaStream([...videoStream.getVideoTracks(),...dest.stream.getAudioTracks()]);
  ...
  try{if(music){await audioCtx.resume();await music.a.play()}await playSequence(true);await wait(150);rec.stop();await done;const type=rec.mimeType||mime||'video/webm',ext=type.includes('mp4')?'mp4':'webm',name=`${safe($('#clipProjectName').value)}.${ext}`,blob=new Blob(chunks,{type});state.last={blob,name};const box=$('#clipBuildDownloads');box.innerHTML=`<div class="downloadItem"><div><h3>${name}</h3><small>${total.toFixed(1)} seconds · ${blendEnabled()?'smooth blends':'hard cuts'} · music ${state.music?'added':'not added'}</small></div><div class="actions"><button id="downloadClipBuildAgain" class="good">Download Video</button></div></div>`;$('#downloadClipBuildAgain').onclick=()=>downloadBlob(blob,name);downloadBlob(blob,name);$('#clipBuildStatus').textContent='Finished video created and downloaded.'}catch(e){if(rec.state!=='inactive')rec.stop();$('#clipBuildStatus').textContent=e.message||'The video could not be created.'}finally{if(music){music.a.pause();URL.revokeObjectURL(music.url);music.a.remove()}await audioCtx.close();b.disabled=false;state.busy=false}
}
function bind(){
  $('#videoClipUpload').onchange=async e=>{const files=[...e.target.files].slice(0,MAX_CLIPS);state.clips=[];$('#clipBuildStatus').textContent='Checking clips…';let used=0,lastMessage='';for(const file of files){try{const duration=await mediaDuration(file,'video');if(duration<MIN_CLIP_SECONDS){lastMessage=`Skipped ${file.name}: the clip is too short.`;continue}if(duration>MAX_CLIP_SECONDS+.05){lastMessage=`Skipped ${file.name}: each clip must be three minutes or shorter.`;continue}if(used+duration>MAX_TOTAL_SECONDS+.05){lastMessage=`Skipped ${file.name}: the clips together cannot exceed three minutes before blending.`;continue}state.clips.push({file,duration});used+=duration}catch(err){lastMessage=err.message}}renderClips();drawEmpty();if(state.clips.length){updateReadyStatus();if(lastMessage)$('#clipBuildStatus').textContent+=` ${lastMessage}`}else $('#clipBuildStatus').textContent=lastMessage||'No usable clips were selected.';e.target.value=''};
...
function init(){addUI();bind();resize();renderClips();document.querySelectorAll('.badge').forEach(el=>{if(el.textContent.includes('Version'))el.textContent='Version 0.8.3'});const footer=$('.footer');if(footer)footer.textContent='Five Oaks Studio — Version 0.8.3'}
